    ...
    if(isset($_POST['vcode']) and $_POST["vcode"] !== ""){
        if(isset($_SESSION['vcode'])){
            ...
            if($vt + $_SESSION["vtime"] <= $time){
                if((int)($vt/10)<=$_SESSION["vkey"] and (int)($vt/5>=$_SESSION["vkey"])){
                    ...
                    if($res->num_rows>0){
                        ...
                        if($conn->query($sql)==true){
                            $sql = "UPDATE user SET point=point+$ap WHERE user='{$_SESSION['user']}'";
                            if($conn->query($sql)==true){
...
if(isset($_SESSION["videos{$type}"])){
        for($i=0;$i<count($_SESSION["videos{$type}"]);$i++){
            ...
            $sql = "SELECT link FROM video WHERE link='{$link}'";
                    ...
                    $sql = "SELECT point FROM user WHERE user='{$user}'";
                        $res1 = $conn->query($sql);
                        if($res1->num_rows>0){
                            $points = $res1->fetch_assoc();
                            if($points["point"]>$vp){
